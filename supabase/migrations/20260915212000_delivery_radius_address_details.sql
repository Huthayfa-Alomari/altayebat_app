...
begin
  ...
  if p_address_id is not null then
    ...
    select a.lat,a.lng,a.city,a.area into v_lat,v_lng,v_city,v_area
  ...
  if v_active_zone_count=0 then
    return jsonb_build_object('serviceable',false,'reason','NO_DELIVERY_ZONE_CONFIGURED','currency',v_store.currency);
  end if;

  if v_lat is null or v_lng is null then
    return jsonb_build_object('serviceable',false,'reason','LOCATION_REQUIRED','currency',v_store.currency);
  end if;
...
declare
  v_address record;
begin
  if new.address_id is null then return new; end if;
...
for each row execute function private.enrich_order_address_snapshot();
...
select
  s.id,'نطاق التوصيل الأساسي','الزرقاء',array[]::text[],
  32.07375,36.09375,15.0,
  coalesce(s.delivery_fee,1.0),coalesce(s.min_order,5.0),35,75,10,true
from public.stores s
where s.id='61e6f35d-7004-4a33-948c-b297ba446678'::uuid
  and not exists (
    select 1 from public.delivery_zones z where z.store_id=s.id and z.is_active=true
  );
