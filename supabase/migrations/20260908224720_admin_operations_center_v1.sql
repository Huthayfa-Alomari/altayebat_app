...
begin
  ...
  from public.store_automation_config c
  ...
  v_today := public.get_daily_report_payload(
    p_store_id,
    (now() at time zone coalesce(v_config->>'timezone','Asia/Amman'))::date
  );
  ...
  return jsonb_build_object(
    'store_id',p_store_id,
    'generated_at',now(),
    'open_alert_count',coalesce(v_open_count,0),
    ...
    'delayed_orders',coalesce(v_delayed,'{}'::jsonb),
...
begin
  ...
  update public.store_automation_config
  set enabled=true,
