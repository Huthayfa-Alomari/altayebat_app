-- Harden the legacy delivery RPCs for the rider approval/presence model.
-- Existing admin-created drivers remain compatible because the rider migration
-- gives them approval_status='approved' by default.
...
begin
  ...
  select d.id, d.name, d.phone, d.is_active, d.approval_status
    into v_driver
  ...
  if not found
     ...
     or v_driver.approval_status <> 'approved' then
...
begin
  ...
  insert into public.driver_tracking_sessions(
    ...
    p_order_id,
    v_driver_id,
    extensions.digest(lower(v_token), 'sha256'),
    auth.uid(),
    v_expires_at
  );
...
begin
  select d.store_id, d.approval_status
    into v_store_id, v_approval_status
  from public.drivers d
  where d.id = p_driver_id;
  ...
  if coalesce(p_active, false) and v_approval_status <> 'approved' then
    raise exception 'RIDER_NOT_APPROVED' using errcode = '22023';
  end if;

  update public.drivers
     set is_active = coalesce(p_active, false),
         availability_status = case
           when coalesce(p_active, false) then availability_status
           else 'offline'
         end,
