...
class _DriverModeScreenState extends State<DriverModeScreen> {
  ...
  bool get _orderClosed => _status == 'delivered' || _status == 'cancelled';
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      setState(() {
        _loading = false;
        _error = 'تعذر فتح مهمة التوصيل. الرابط منتهي أو غير صالح.';
      });
    }
  }
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      settings = AndroidSettings(
        ...
        foregroundNotificationConfig: ForegroundNotificationConfig(
          ...
          notificationText: 'يتم تحديث موقعك للزبون أثناء مهمة التوصيل.',
  ...
  Future<void> _openNavigation() async {
    ...
    final lat = _findNumber(address, const ['lat', 'latitude', 'delivery_lat']);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(_error ?? 'تعذر فتح المهمة.', textAlign: TextAlign.center),
            const SizedBox(height: 16),
            FilledButton(
  ...
  static double? _findNumber(Map<String, dynamic> map, List<String> keys) {
    ...
    for (final value in map.values) {
      if (value is Map) {
        final nested = _findNumber(Map<String, dynamic>.from(value), keys);
