...
class SupabaseService {
  ...
  static Never _throwFriendlyCheckoutError(PostgrestException error) {
    ...
    if (message.contains('INSUFFICIENT_STOCK') ||
        message.contains('STOCK_CHANGED')) {
    ...
    if (message.contains('INVALID_PAYMENT_METHOD')) {
      throw StateError('طريقة الدفع غير مدعومة');
    }
    if (message.contains('INVALID_SUBSTITUTE_POLICY')) {
      throw StateError('خيار البدائل غير صالح');
    }
    if (message.contains('MIN_ORDER')) {
      throw StateError('قيمة السلة أقل من الحد الأدنى للطلب');
    }
    if (message.contains('EMPTY_CART')) {
  ...
  static Future<String> startCardPayment(String orderId) async {
    try {
      ...
      final data = response.data;
      if (response.status < 200 || response.status >= 300) {
        final details = data?.toString().toUpperCase() ?? '';
        if (details.contains('PAYTABS_NOT_CONFIGURED') ||
            details.contains('NOT CONFIGURED')) {
  ...
  static Future<bool> cancelUnstartedCardOrder(String orderId) async {
    try {
      final result = await _client.rpc(
  ...
  static Future<Map<String, dynamic>> getStoreContactConfig() async {
    final paymentConfig = await getStorePaymentConfig();
    final data = await _client
        .from('stores')
        .select('name')
